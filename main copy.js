document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateActiveNavLink() {
        ...
        navLinks.forEach(link => {
            ...
            else if (linkPath.includes('/examples.html') && currentPath.startsWith('/examples/')) {
                link.classList.add('active');
                link.setAttribute('aria-current', 'page');
            }
        });
    ...
    // --- ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (АНИМАЦИЯ, ЗАГРУЗКА) ---
    ...
    function slideDown(element) {
        ...
        element.addEventListener('transitionend', () => { element.style.height = null; }, { once: true });
    }
    ...
    // --- ТОЧКА ВХОДА: ЗАГРУЗКА КОМПОНЕНТОВ И ЗАПУСК ЛОГИКИ ---
    const isExamplesPage = window.location.pathname.includes('/examples/');
    const basePath = isExamplesPage ? '../' : '';

    // Загружаем НАВИГАЦИЮ и после этого обновляем активную ссылку
    loadComponent(`${basePath}nav.html`, 'navbar-placeholder').then(success => {
        if (success) {
            updateActiveNavLink();
        }
    });
